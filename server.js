...
const findRestaurants = (db, callback) => {
	...
	const findDocuments = (db,details, callback) => {
		let cursor = db.collection('restaurants').find({"restaurant_id": details }).limit(10);
		cursor.forEach((doc) => {
	...
	const findMap = (db,map, callback) => {
		let cursor = db.collection('restaurants').find({"restaurant_id": map }).limit(10);
		cursor.forEach((doc) => {
		console.log(JSON.stringify(doc));
		callback(doc);	
		});
	...
	const updateRating = (db, doc) => {
		db.collection('restaurants').
		update({"restaurant_id": doc.restaurant_id},
					{$push: {"grades" : {
...
app.get('/createPage', (req,res) => {
		...
		const db = client.db(dbName);
		findRestaurants(db,(restaurants) => {
		var number = restaurants.length +1;
		//console.log("Disconnected MongoDB server");
